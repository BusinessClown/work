...
def crack(password_file):
    ...
    def check(pwd):
        ...
        if h in hashes and h not in cracked:
            cracked[h] = pwd
            print(f"[+] User {hashes[h]}: {pwd}")
            return True
        return False
    
    # Attack 1: Digits 1-8
    for length in range(1, 9):
        if len(cracked) == len(hashes): break
        for combo in itertools.product('0123456789', repeat=length):
            if check(''.join(combo)) and len(cracked) == len(hashes): break
    ...
    if os.path.exists('dictionary.txt'):
        ...
        for word in words:
            if check(word.lower()) and len(cracked) == len(hashes): break
        
        priority = sorted([w for w in words if len(w) <= 14], key=lambda x: (len(x), x))
        for w1, w2 in itertools.product(priority, repeat=2):
            combo = (w1 + w2).lower()
            if 6 <= len(combo) <= 28:
                if check(combo) and len(cracked) == len(hashes): break
        
        for word in words:
            if len(cracked) == len(hashes): break
            for digits in range(0, 10000):
                if check(word.lower() + str(digits)) and len(cracked) == len(hashes): break
    
    # Results
    elapsed = time.time() - start
    print(f"\n{'='*60}")
    print(f"Cracked: {len(cracked)}/{len(hashes)} ({100*len(cracked)/len(hashes):.0f}%)")
